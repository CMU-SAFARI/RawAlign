...
static ko_longopt_t long_options[] = {
	...
	{ (char*)"dtw-evaluate-chains",		ko_no_argument,			323 },
	{ (char*)"dtw-output-cigar",		ko_no_argument,			324 },
	{ (char*)"dtw-border-constraint",	ko_required_argument,	325 },
	{ (char*)"dtw-log-scores",			ko_no_argument,			326 },
	{ (char*)"no-chainingscore-filtering",	ko_no_argument,		327 },
	{ (char*)"dtw-match-bonus",			ko_required_argument,	328 },
	{ (char*)"output-chains",			ko_no_argument,			329 },
	{ (char*)"dtw-fill-method",			ko_required_argument,	330 },
	{ (char*)"dtw-min-score", 			ko_required_argument, 	331 },
	{ (char*)"log-anchors",				ko_no_argument,			332 },
	{ (char*)"log-num-anchors",			ko_no_argument,			333 },
	{ 0, 0, 0 }
};
...
int main(int argc, char *argv[])
{
	...
	while ((c = ketopt(&o, argc, argv, 1, opt_str, long_options)) >= 0) {
		...
		else if (c == 323) opt.flag |= RI_M_DTW_EVALUATE_CHAINS; // --dtw-evaluate-chains
		else if (c == 324) opt.flag |= RI_M_DTW_OUTPUT_CIGAR; // --dtw-output-cigar
		else if (c == 325) { //--dtw-border-constraint
			if(ri_mapopt_parse_dtw_border_constraint(&opt, o.arg) != 0){
				fprintf(stderr, "[ERROR] unknown DTW border constraint in \"%s\"\n", argv[o.i - 1]);
				return 1;
			}
		}
		else if (c == 326) opt.flag |= RI_M_DTW_LOG_SCORES; // --dtw-log-scores
		else if (c == 327) opt.flag |= RI_M_DISABLE_CHAININGSCORE_FILTERING; // --no-chainingscore-filtering
		else if (c == 328) opt.dtw_match_bonus = atof(o.arg); // --dtw-match-bonus
		else if (c == 329) opt.flag |= RI_M_OUTPUT_CHAINS; // --output-chains
		else if (c == 330) { //dtw-fill-method
			if(ri_mapopt_parse_dtw_fill_method(&opt, o.arg) != 0){
				fprintf(stderr, "[ERROR] unknown DTW fill method in \"%s\"\n", argv[o.i - 1]);
				return 1;
			}
		}
		else if (c == 331) opt.dtw_min_score = atof(o.arg); // --dtw-min-score
		else if (c == 332) opt.flag |= RI_M_LOG_ANCHORS; // --log-anchors
		else if (c == 333) opt.flag |= RI_M_LOG_NUM_ANCHORS; // --log-num-anchors
		else if (c == 'V') {puts(RI_VERSION); return 0;}
	}

	if (argc == o.ind || fp_help == stdout) {
		fprintf(fp_help, "Usage: rawalign [options] <target.fa>|<target.idx> [query.fast5] [...]\n");
		...
		fprintf(fp_help, "    --dtw-evaluate-chains     evaluate chains using DTW [%s]\n", opt.flag & RI_M_DTW_EVALUATE_CHAINS? "yes" : "no");
		...
		fprintf(fp_help, "    --dtw-log-scores     log DTW scores [%s]\n", opt.flag & RI_M_DTW_LOG_SCORES? "yes" : "no");
		fprintf(fp_help, "    --dtw-match-bonus FLOAT     DTW match bonus FLOAT [%g]\n", opt.dtw_match_bonus);
		fprintf(fp_help, "    --output-chains	 output chain anchors [%s]\n", opt.flag & RI_M_OUTPUT_CHAINS? "yes" : "no");
		fprintf(fp_help, "    --log-anchors	 log chain anchors [%s]\n", opt.flag & RI_M_LOG_ANCHORS? "yes" : "no");
		fprintf(fp_help, "    --log-num-anchors	 log number of chain anchors [%s]\n", opt.flag & RI_M_LOG_NUM_ANCHORS? "yes" : "no");
		fprintf(fp_help, "    --no-chainingscore-filtering     disable filtering of chains based on chaining score [%s]\n", opt.flag & RI_M_DISABLE_CHAININGSCORE_FILTERING? "yes" : "no");
		fprintf(fp_help, "\n  ONT Device:\n");
