...
def plot(tputs, latency, accs, sequencing_latencies, memory_footprints, plottitle="Read Mapping Spider Plot"):
    ...
    for toolname in toolnames:
        ...
        if 'band_radius' not in rawalign_config or rawalign_config['band_radius'] != 0.10: continue
        if 'dtwmatchbonus' not in rawalign_config: continue
        if 'dtwminscore' not in rawalign_config: continue

        matchbonus = rawalign_config['dtwmatchbonus']
        minscore = rawalign_config['dtwminscore']

        if minscore not in xs:
            xs[minscore] = []
            f1_ys[minscore] = []
            precision_ys[minscore] = []
            recall_ys[minscore] = []

        xs[minscore].append(matchbonus)
        f1_ys[minscore].append(accs[toolname][2])
        precision_ys[minscore].append(accs[toolname][0])
        recall_ys[minscore].append(accs[toolname][1])


    fig, ax = plt.subplots(figsize=(6, 3))

    #ax.set_title(plottitle)
    ax.set_xlabel("Match Bonus",        weight='bold', fontsize=12)
    ...
    minscore = 20.0
    datapoints = zip(xs[minscore], f1_ys[minscore], precision_ys[minscore], recall_ys[minscore])
    datapoints = sorted(datapoints, key=lambda x: x[0])
    xs, f1_ys, precision_ys, recall_ys = zip(*datapoints)

    l0 = ax.plot(xs, f1_ys,         color=metric_name_to_color('f1'),           linestyle='-', linewidth=major_linewidth, label='F-1 Score')
    l1 = ax.plot(xs, precision_ys,  color=metric_name_to_color('precision'),    linestyle='--', linewidth=minor_linewidth, label='Precision')
    l2 = ax.plot(xs, recall_ys,     color=metric_name_to_color('recall'),       linestyle='--', linewidth=minor_linewidth, label='Recall')
...
#    )
    ...
    memory_footprints = filter_metrics(parse_memory_footprints(results), main_rawalign_filter)
    plot(tputs, latency, accs, bp_latencies, memory_footprints,
         plottitle='Match Bonus Parameter Sweep on d2 E.coli',
    )
