...
def get_latex_table_single_tabular_contents(dataset_results, dataset_name = '', omit_metric_names=False, omit_first_line=False):
    ...
    metric_names = set(['Memory\nFootprint (GB)', 'Throughput\n(bp/s)', 'Analysis\nLatency (ms)', 'Sequencing\nLatency (Chunks)', 'Accuracy\n(F-1)'])
    ordered_metric_names = ['Memory\nFootprint (GB)', 'Throughput\n(bp/s)', 'Analysis\nLatency (ms)', 'Sequencing\nLatency (Chunks)', 'Accuracy\n(F-1)']
...
if __name__ == "__main__":
    ...
    write_latex_table(total_results, Path('paperfigures/numeric_results_table.tex'))
