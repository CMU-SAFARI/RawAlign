...
typedef struct ri_chain_s{
  ...
  bool operator>(const ri_chain_s &b) const {
	//this is assuming that alignment_score is always 0 when not using dtw
    return std::tie(alignment_score, chaining_score, n_anchors, strand, reference_sequence_index, start_position, end_position) >
           std::tie(b.alignment_score, b.chaining_score, b.n_anchors, b.strand, b.reference_sequence_index, b.start_position, b.end_position);
  }
}ri_chain_t;
...
typedef struct pipeline_ms{
	...
	int su_stop;
	std::vector<ri_sig_t> signals;
	std::vector<ri_events_t> events;
} pipeline_mt;
...
/**
 * Map raw nanopore signals of a single file to a reference genome
 *
